...
_FLAG_ANDROID_SDK_DIR = flags.DEFINE_string(
  ...
  "(default: the value of the ANDROID_HOME environment variable, or "
...
class SanitizerSetup:
  ...
  def tweaked_cmakelists_txt_lines(self, path: pathlib.Path) -> Iterable[str]:
    ...
    with path.open("rt", encoding="utf8") as f:
      for line in f:
        if line.strip().startswith("cmake_minimum_required("):
        ...
        elif (line.strip().startswith(add_executable_token) or
            line.strip().startswith(add_library_token)):
          yield line
          executable_token_start = line.find(add_executable_token)
          library_token_start = line.find(add_library_token)
          if executable_token_start >= 0 and library_token_start >= 0:
            raise self.Error(
              "Unable to determine if this line is an executable or a library: "
              + line.strip()
            )
          elif executable_token_start >= 0:
            target_name_start = (executable_token_start +
              len(add_executable_token))
          elif library_token_start >= 0:
            if " IMPORTED" in line:
              continue
            target_name_start = library_token_start + len(add_library_token)
          else:
            raise AssertionError(f"should never get here: line={line.strip()}")
          ...
          if target_name not in ("libuWS", "native_app_glue"):
            sanitize_target_names.append(target_name)
        else:
          yield line

    for sanitize_target_name in sanitize_target_names:
      yield (
        f"target_compile_options({sanitize_target_name} "
        "PUBLIC -fsanitize=address -fno-omit-frame-pointer)\n"
      )
      yield (
        f"set_target_properties({sanitize_target_name} "
    ...
    for (arch_dir_name, lib_filename) in tuple(ndk_libs.items()):
      ...
      if len(paths) == 0:
        ...
        raise self.Error(
          ...
          f"but expected exactly 1: {', '.join(str(path) for path in paths)}")
      else:
  ...
  def tweaked_ndk_wrap_sh_lines(self, path: pathlib.Path) -> Iterable[str]:
    ...
    if not asan_options_found:
      raise self.Error(f"ASAN_OPTIONS not found in {path}")
